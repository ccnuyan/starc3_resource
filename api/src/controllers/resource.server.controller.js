...
exports.search = function(req, res, next) {
  ...
  if (req.query.knowledgeNode) {
    ...
    filter.and.push({
      term: {
        knowledgeNodeId: req.query.knowledgeNode
      }
    });
  }
...
exports.decision = function(req, res, next) {
  ...
  if (resource.status === 'submitted') {
    if (req.body.status === 'rejected') {
      ...
      return next({
        message: 'you should decide to reject or approve this resource'
      });
    }
  } else {
    return next({
    ...
    swiftInitializer.init(function(err, swift) {
      ...
      swift.copyObject(cloudBox, fileObject._id, container, sourceFileObject._id, function(opsErr, opsRet) {
        if (opsErr || opsRet.statusCode !== 201) {
          return next({
            message: 'cloud storage copy object error'
          });
        }
...
exports.import = function(req, res, next) {
  ...
  if (req.file.mimetype !== 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') {
    return next({
      message: 'only *.xlsx file allowed here'
    });
  }
...
exports.download = function(req, res, next) {
  ...
  if (resource.res_meta_type !== 'file') {
    return next({
      message: 'downloading is not allowed for this resource meta type'
    });
  }
  ...
  swiftInitializer.init(function(err, swift) {
    ...
    swift.getFile(cloudBox, resource.fileObject.id, function(err) {
      if (err) {
        return next({
          message: 'cloud storage get file error'
        });
      } else {
...
exports.edit = function(req, res, next) {
  ...
  if (resource.status === 'submitted' || resource.status === 'rejected') {
    ...
    return next({
      message: 'resource status : ' + req.resource.statu
    });
  }
};
...
exports.mark = function(req, res, next) {
  var resource = req.resource;
  return next({
    message: 'resource mark : not implemented'
  });
};
...
exports.resourceByID = function(req, res, next, id) {
  Resource.findById(id)
    ...
    .exec(function(err, resource) {
      ...
      if (!resource) {
        return next({
          message: 'resource specified does not exist'
        });
      }
